...
def per_class_group_split(image_paths, image_labels, cluster_map, test_ratio=0.40, random_state=42):
...
def plot_feature_importance(best_result, feature_names, output_dir):
    ...
    plt.title(f'Top 20 Feature Importances - {best_result["name"]}', fontsize=14, fontweight='bold')
...
def train_and_compare_classifiers(dataset_dir, output_dir="results"):
    ...
    print(f"  Actual {class_names[0]:<8} {cm[0,0]:6d}   {cm[0,1]:6d}")
    print(f"         {class_names[1]:<8} {cm[1,0]:6d}   {cm[1,1]:6d}")
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"  Train/Test Split: 60%/40%\n")
